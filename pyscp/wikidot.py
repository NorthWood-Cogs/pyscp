...
class Page(pyscp.core.Page):
    ...
    async def _module(self, *args, **kwargs):
        """Call Wikidot module."""
        return await self._wiki._module(*args, page_id=self._id, **kwargs)

    async def _action(self, event, **kwargs):
    ...
    async def history(self):
        ...
        for row in reversed(soup('tr')[1:]):
            ...
            await pyscp.core.Revision(rev_id, number, User(user), time, comment)

    @pyscp.utils.cached_property
    async def votes(self):
        """Return all votes made on the page."""
        data = await self._module('pagerate/WhoRatedPageModule')['body']
        soup = bs4.BeautifulSoup(data, 'lxml')
    ...
    async def source(self):
        data = (await self._module('viewsource/ViewSourceModule'))['body']
        soup = bs4.BeautifulSoup(data, 'lxml')
    ...
    async def files(self):
        ...
        data = await self._module('files/PageFilesModule')['body']
        soup = bs4.BeautifulSoup(data, 'lxml')
    ...
    async def edit(self, source, title=None, comment=None):
...
class Thread(pyscp.core.Thread):
    ...
    async def posts(self):
        ...
        for post, parent in crawl_posts(posts):
            ...
            await pyscp.core.Post(post_id, title, content, user, time, parent)
...
class Wiki(pyscp.core.Wiki):
    ...
    async def _module(self, _name, **kwargs):
        """
    ...
    async def _pager(self, _name, _key, _update=None, **kwargs):
        ...
        for idx in range(2, int(counter.text.split(' ')[-1]) + 1):
            ...
            await self._module(_name, **kwargs)

    async def _list_pages_raw(self, **kwargs):
        """
    ...
    async def _list_pages_parsed(self, **kwargs):
        """
    ...
    async def list_categories(self):
        ...
        for elem in [e.parent for e in soup(class_='name')]:
            ...
            await pyscp.core.Category(
                cat_id, title, description, int(size))

    async def list_threads(self, category_id):
        ...
        for elem in itertools.chain(*elems):
            ...
            await self.Thread(self, thread_id, title, description)
    ...
    async def list_images(self):
        ...
        for elem in elems:
            ...
            await pyscp.core.Image(url, source, status, notes, None)
...
async def crawl_posts(post_containers, parent=None):
    """
    ...
    for container in post_containers:
        await container.find(class_='post'), parent
        await crawl_posts(
